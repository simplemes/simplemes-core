...
===== fieldOrder and Sub-Classes

When a parent class has a `fieldOrder` value, all the child classes should have `fieldOrder`,
...
This happens often when the child class has only a reference to some containing class that is not
displayed. The FlexField example above has only a reference to the containing _FlexType_ class,
which we don't normally display.

The link:groovydoc/org/simplemes/eframe/domain/DomainUtils.html[DomainUtils^] has methods to
determine the effective field order for a domain that takes into account the super-classes.
