...
| *value* | The initial value for the field.  Ignored if *valueName* is given.
| *valueName* | The name of the element in the display model that holds the value object.
...
----
<@efForm id="logRMA">
  <@efField field="RMA.rmaType" id="rmaType" valueName="rmaDataModel"/>  // <.>
<@/efForm>
----
...
StandardModelAndView logRMADialog(HttpRequest request, @Nullable Principal principal) {
  ...
  modelAndView['rmaDataModel'] = new RMA(rmaType: flexType)  // <.>
