...
but they miss out in key areas.  This includes dealing with hibernate references and other
related issues.  These issues are mostly hidden from you when you use the built-in <<Rest API>>
support for this framework.  It handles most of the strange hibernate issues and supports the
...
// TODO: Verify these are still issues.

* Create/Read/Update/Delete (CRUD).
* Child records.
* Date/time encoding (ISO).
* Child record removal.
* Clean key field handling (no Micronaut IDs needed).
* Support for optional Micronaut ID in references to other objects.
