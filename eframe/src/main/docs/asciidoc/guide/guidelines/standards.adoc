...
[inline-toc]#<<Groovy Features to Use or Avoid>>#
...
Groovy allows untyped fields/variables in your classes.  This is great for internal uses.
However, all APIs should use proper Java-style typing where possible.  This means method
parameters should not use *def* or *Object* when a real type is known.
...
Groovy is sometimes called 'a better Java'.  It has a lot of features that we use to enhance
clarity.  It does have some features that we avoid because they are not very clear:

.Groovy Features to Avoid
...
These are just a few of the useful features that make things clear in Groovy.
There is an excellent series of blog posts
link:https://blog.mrhaki.com/search/label/Groovy[Groovy Goodness] that covers many of
the features of Groovy.
