...
NOTE: The other panels will still have the current activities active while the non-GUI
...
NOTE: If an activity defines a .execute() method and uses the `<@efForm>`, then the execute()
