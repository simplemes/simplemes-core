...
You dashboard can have common buttons defined by you or your users.  Each button will trigger one
...
----
<div id="wcSelection">
  <@efField model="SelectionCommand" field="workCenter"/>
  <span id="DashboardButtons"></span>
</div>
----

The first element with the ID _'DashboardButtons'_ will be replaced with the configurable
buttons for this dashboard. You should only have one activity on the dashboard with this
`DashboardButtons` ID.
