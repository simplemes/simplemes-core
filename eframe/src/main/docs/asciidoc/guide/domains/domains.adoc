...
This will call the `encryptPassword()` before every save.  
...
This will always be called from within a transaction.
...
You can use the _@Transactional_ annotation or the `withTransaction()` method.
