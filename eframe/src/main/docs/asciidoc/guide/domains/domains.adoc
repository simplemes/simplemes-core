...
| *@Column*|JPA/Jakarta| Defines the column name, length and nullable flag.
| *@Nullable*|Java| Marks a field as nullable.  @Column is a better choice.
...
| *@ID*|Micronaut| Marks a field as the ID field.  UUID is supported.
...
* Extends: BaseRepository, CrudRepository<__Domain__, UUID>
...
<.> Uses Postgres for production and dev environments.

===== FindByUuid and Foreign References
...
domain, then it will be executed by the domain's `save()` method.  For example:

[source,groovy]
.Example - beforeSave Method on User
----
def beforeSave() {
  encryptPassword()
}
...
This will call the `encryptPassword()` before every save.  If you need to detect insert vs. update
you should look at the object's uuid.  If null, then it is an insert scenario.
...
----
<.> Defines the reference as a simple foreign reference.  Not as a <<parent-child-one-to-many>>
    relationship.
...
NOTE: This _targetEntity_ is used as a marker for a simple foreign reference.  Do not use
      for this _targetEntity_ option for <<parent-child-one-to-many>> relationships.
...
to the parent and to the related record.  You must use the _@ManyToMany_ annotation (from Jakarta)
and add a _@Join_ annotation to the `findByXYZ()` method in your repository.

Also, the _@DomainEntity_ annotation adds the save() and delete() logic to handle this join table.

NOTE: This is sometimes referred to as a list of foreign references.
...
----
<.> An empty list is defined for new instances.  The `findByUserName()` method will create
    the correct array if the finder is correct.
