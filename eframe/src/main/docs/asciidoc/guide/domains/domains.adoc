...
Parent/Child relationships are quite common in enterprise applications.  The children are fully-owned by the parent.  Deleting
the parent also deletes the children.  A typical parent/child relationship should be defined as:
...
The use of a _List_ preserves the order of the child elements and allows updates using inline
grids within the GUI markers in the framework.  When a user deletes an entry from the list in the
GUI, the controller must be able to determine that
a record was removed.  If you leave out the _List_, then a Set is used.  In that case, the
framework can't handle all update scenarios using inline grids. If a _Set_ must be used,
then you can't use the inline grid options.

For these updates to work, you will have to adjust the generated controller code for these
scenarios.  See <<controllers>> for details.

// TODO: Investigate issues with GORM bug 874.  See domain.adoc in grails eframe.  Also break into multiple docs.
