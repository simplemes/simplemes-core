...
<.> The field `rmaType` will hold the reference to the appropriate flex type.  The actual
    values will be stored in the `customFields` field as described
...
response.customFields = rma.customFields

----
<.> You need to copy the flex type field and the values holder (customFields).
