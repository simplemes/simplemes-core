...
<script>
  ${params._variable}.execute =  function() {
    ...
    if (order) {
      ...
      dashboard.postActivity(startRequest,'/work/reverseStart','${params._panel}',{success: function(response) {
        var json = JSON.parse(response);
        var order = json[0].order;
        var qty = json[0].qty;
        var msg = ef.lookup('reversedStart.message',order,qty);
        ef.displayMessage(msg);
        var list = [{order: order}];
        var event = {type: 'ORDER_LSN_STATUS_CHANGED', source: '/work/reverseStartActivity', list: list};
  ...
  <@efPreloadMessages codes="reversedStart.message,orderLSN.missing.message"/>
</script>