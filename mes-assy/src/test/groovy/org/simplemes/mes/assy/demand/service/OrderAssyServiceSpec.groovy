...
class OrderAssyServiceSpec extends BaseSpecification {
  ...
  @Rollback
  def "verify that findAssembledComponents works"() {
    given: 'a released order with multiple records assembled'
    def order = AssyUnitTestUtils.releaseOrder(components: ['CPU'])
    def comp1 = AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])
    def comp2 = AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])
    def comp3 = AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])

    and: 'a title is set for the component'
    comp1.component.title = 'title'
    comp1.component.save()

    when: 'the components are searched'
    def orderComponentState = service.findAssembledComponents(order)
    //println "orderComponentState = $orderComponentState"

    then: 'the returned values are correct'
    orderComponentState.size() == 3
    orderComponentState[0].sequence == comp1.sequence
    orderComponentState[0].component == 'CPU'
    orderComponentState[0].componentAndTitle == TypeUtils.toShortString(comp1.component, true)
    orderComponentState[0].qtyRequired == 1.0
    orderComponentState[0].qtyAssembled == 1.0
    orderComponentState[0].sequencesForRemoval == [comp1.sequence]

    orderComponentState[1].sequence == comp2.sequence
    orderComponentState[1].component == 'CPU'
    orderComponentState[1].qtyAssembled == 1.0

    orderComponentState[2].sequence == comp3.sequence
    orderComponentState[2].component == 'CPU'
    orderComponentState[2].qtyAssembled == 1.0
  }

  @Rollback
  def "verify that findAssembledComponents works with assembly data display"() {
    given: 'a flex type for the assy data'
    def flexType = DataGenerator.buildFlexType(flexType: 'ASSEMBLY')

    and: 'a released order with a component assembled'
    def order = AssyUnitTestUtils.releaseOrder(components: ['CPU'])
    AssyUnitTestUtils.assembleComponent(order, [bomSequence       : 10, assemblyDataType: flexType,
                                                assemblyDataValues: [FIELD1: 'ACME', ASSEMBLY1: '2017103']])
  ...
  @Rollback
  def "verify that findAssembledComponents works with LSN"() {
    given: 'a released order with LSN and a component assembled'
    def order = AssyUnitTestUtils.releaseOrder(components: ['CPU'],
                                               lsnTrackingOption: LSNTrackingOption.LSN_ONLY,
                                               qty: 3.0, lotSize: 3.0)
    def lsn = order.lsns[0] as LSN
    def comp1 = AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10, lsn: lsn])

    when: 'the components are searched'
    def orderComponentState = service.findAssembledComponents(lsn)
    //println "orderComponentState = $orderComponentState"
  ...
  @Rollback
  def "verify that findAssembledComponents works with a list of sequences to filter by"() {
    given: 'a released order with multiple records assembled'
    def order = AssyUnitTestUtils.releaseOrder(components: ['CPU'])
    def comp1 = AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])
    AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])
    AssyUnitTestUtils.assembleComponent(order, [bomSequence: 10])

    when: 'the components are searched'
    def orderComponentState = service.findAssembledComponents(order, [comp1.sequence])
    //println "orderComponentState = $orderComponentState"
  ...
  @Rollback
  def "verify that findAssembledComponents handles no demand argument"() {
    when: 'the components are searched'
    def orderComponentState = service.findAssembledComponents(null)
    //println "orderComponentState = $orderComponentState"

    then: 'the returned values are correct'
    orderComponentState.size() == 0
  }
